...
gasolina = Item(tela.get_resolucao()[0] / 2, - 100)
...
terra = Nova_nave('arquivos/terra.png', 800, 50)
...
# quando a nave explode reinicia o menu
def reinicia_menu():
...
while jogoAtivo:
    ...
    if tela.get_game_loop() == True:
        ...
        if tela.get_cronometro()[4] > 3:
            ...
            gasolina.set_volume_item(pos_bola_slider_volume/100)
            ...
            nave.set_volume_propulsor(pos_bola_slider_volume/100)
            nave.set_volume_explosao(pos_bola_slider_volume/1000)
            nave.set_altitude(abs(nave.get_posicao_y() - tela.get_resolucao()[1]))
            nave.update()
    ...
    # nada como um bom print para debur rsrsrs
    if debug:
